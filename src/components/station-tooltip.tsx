...
function getRegionBadge(regionId: string | undefined) {
	switch (regionId) {
		case "70":
			return { label: "Jersey City", variant: "jerseyCity" as const };
		case "71":
			return { label: "NYC", variant: "nyc" as const };
		case "311":
			return { label: "Hoboken", variant: "hoboken" as const };
		default:
			return null;
	}
}
...
function StationBadges({ station }: StationTooltipProps) {
	const regionBadge = getRegionBadge(station.region_id);
	const isActive = getIsActive(station);
